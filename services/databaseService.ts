...
import { Project, Message, Plan, ProjectPlatform, Profile, Chat, ChatMode, Memory, ProjectType, MemoryLayer, AppSettings, ChatWithProjectData, Friendship, Notification } from '../types';
...
const handleSupabaseError = (error: any, context: string): never => {
    ...
    if (message.includes('schema cache') || message.includes('cache lookup failed')) {
...
const sanitizeMessagePayload = (message: Partial<Message>) => {
    const payload: any = { ...message };
    
    // Map camelCase to snake_case or lowercase for DB
    if ('groundingMetadata' in payload) {
        if (payload.groundingMetadata) {
            payload.grounding_metadata = payload.groundingMetadata;
        }
        delete payload.groundingMetadata;
    }
    
    // Map imageStatus to imagestatus (Supabase columns are lowercase)
    if ('imageStatus' in payload) {
        if (payload.imageStatus) {
            payload.imagestatus = payload.imageStatus;
        }
        delete payload.imageStatus;
    }

    // Remove internal fields that shouldn't be sent to the DB
    if ('memoryToCreate' in payload) {
        delete payload.memoryToCreate;
    }

    // Remove undefined fields to avoid sending columns that don't exist
...
export const getMessages = async (supabase: SupabaseClient, chatId: string): Promise<Message[]> => {
    ...
    // Fix: Map snake_case from DB to camelCase for app
    return (data || []).map((msg: any) => ({
        ...msg,
        groundingMetadata: msg.grounding_metadata || msg.groundingMetadata,
        imageStatus: msg.imagestatus || msg.imageStatus
    }));
};

export const addMessage = async (supabase: SupabaseClient, message: Omit<Message, 'id' | 'created_at'>): Promise<Message> => {
    // Use the sanitizer to clean and map fields before insertion
    const messageToInsert = sanitizeMessagePayload(message);
    ...
    // Fallback for schema sync issues: if insertion fails specifically due to new columns, try without them.
    // This handles cases where the DB schema hasn't been updated or the cache is stale.
    if (error) {
        const errMsg = getErrorMessage(error);
        if (errMsg.includes('grounding_metadata') || errMsg.includes('imagestatus') || errMsg.includes('schema cache') || errMsg.includes('column')) {
            console.warn("Database schema mismatch detected. Retrying message insertion without new columns.", error);
            
            const fallbackPayload = { ...messageToInsert };
            delete fallbackPayload.grounding_metadata;
            delete fallbackPayload.imagestatus;
            delete fallbackPayload.raw_ai_response;
    ...
    // Fix: Map back to camelCase for frontend
    return {
        ...data,
        groundingMetadata: data.grounding_metadata || data.groundingMetadata,
        imageStatus: data.imagestatus || data.imageStatus
    };
};

export const updateMessage = async (supabase: SupabaseClient, messageId: string, updates: Partial<Message>): Promise<Message> => {
    const updatesToInsert = sanitizeMessagePayload(updates);
    ...
    // Fallback for schema sync issues
    if (error) {
        const errMsg = getErrorMessage(error);
        if (errMsg.includes('grounding_metadata') || errMsg.includes('imagestatus') || errMsg.includes('schema cache') || errMsg.includes('column')) {
             console.warn("Database schema mismatch detected. Retrying message update without new columns.", error);
             
             const fallbackUpdates = { ...updatesToInsert };
             delete fallbackUpdates.grounding_metadata;
             delete fallbackUpdates.imagestatus;
             delete fallbackUpdates.raw_ai_response;
...
export const deleteUser = async (supabase: SupabaseClient, userId: string): Promise<void> => {
    // Deleting from the profiles table.
...
export const getFriendships = async (supabase: SupabaseClient, userId: string): Promise<Friendship[]> => {
    ...
    return (data || []).map((f: any) => ({
        ...f,
        other_user: f.user_id === userId ? f.receiver : f.sender
    }));
};
